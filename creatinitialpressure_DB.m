function initialpressure_DB=creatinitialpressure_DB(Model, Medium, Geometry,absorption_coefficient)
...
for i=1:round(Geometry.cuvette_length/Model.dx)
   for j=1:round(Geometry.cuvette_diameter/(2*Model.dy))
       ...
       p_0(H_1+i,j)=Intensity(i,j)*Model.amplitude*absorption_coefficient*Model.dx*exp(-absorption_coefficient*light_path);%Tophat
    end
end
